...
class DatastoreAPI():
    ...
    def search(self, query, n_docs=3, additional_metadata=[]):
        query_embedding = self.embed_query(query)
        try:
            searched = self.index.search(query_embedding, n_docs, additional_metadata)
        except TypeError:
            # Some indexers (e.g., IVFFlat/IVFPQ) don't accept additional_metadata.
            searched = self.index.search(query_embedding, n_docs)

        if isinstance(searched, tuple) and len(searched) == 4:
            searched_scores, searched_passages, db_ids, metadata = searched
        else:
            searched_scores, searched_passages, db_ids = searched
            metadata = []
