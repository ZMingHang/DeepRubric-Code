...
class IVFPQIndexer(object):
    ...
    def __init__(self,
        ...
        # use_faiss_gpu = os.getenv("USE_FAISS_GPU", "1") != "0"
        use_faiss_gpu = 1
        if use_faiss_gpu:
            try:
    ...
    def load_embeds(self, shard_id=None):
        ...
        for embed_path in self.embed_paths:
            ...
            offset += len(ids)
        if not all_embeds:
            raise ValueError(f"No embeddings matched shard_id={shard_id}. embed_paths={self.embed_paths}")
    ...
    def _add_keys(self, index_path, trained_index_path):
        cpu_index = faiss.read_index(trained_index_path)
        assert cpu_index.is_trained and cpu_index.ntotal == 0
        ...
        start_time = time.time()
        # NOTE: the shard id is an absolute id defined in the file name.
        for shard_idx, embed_path in enumerate(self.embed_paths, 1):
            filename = os.path.basename(embed_path)
            match = re.search(r"passages_(\d+)\.pkl", filename)
            if match is None:
                raise ValueError(f"Cannot parse shard id from embedding file: {embed_path}")
            shard_id = int(match.group(1))

            to_add = self.get_embs(shard_id=shard_id)
            total = len(to_add)
            for start in range(0, total, self.num_keys_to_add_at_a_time):
                end = min(start + self.num_keys_to_add_at_a_time, total)
                batch = np.ascontiguousarray(to_add[start:end])
                index.add(batch)
                print(f"shard {shard_id}: {end}/{total}")

            ids_toadd = [[shard_id, chunk_id] for chunk_id in range(total)]  # TODO: verify chunk_id mapping
            self.index_id_to_db_id.extend(ids_toadd)
            print(f"Added {shard_idx} / {len(self.embed_paths)} shards, ({(time.time() - start_time) / 60:.1f} min)")
...
def test_build_multi_shard_dpr_wiki():
    ...
    embed_dir = os.path.join(data_root, "embeddings/facebook/contriever-msmarco/dpr_wiki/1-shards")
    ...
    index_dir = os.path.join(embed_dir, "index_ivf_pq")
    ...
    passage_dir = os.path.join(data_root, "passages/dpr_wiki/1-shards")
    index = IVFPQIndexer(
        embed_paths,
        index_path,
        meta_file,
