...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    # Aggregate selected reward components from reward_extra_info (stored in non_tensor_batch).
    ...
    for key in reward_component_keys:
        raw_values = batch.non_tensor_batch.get(key, None)
        ...
        if values.size == 0:
            continue
        result[f"reward_components/{key}/mean"] = float(values.mean())
        result[f"reward_components/{key}/max"] = float(values.max())
        result[f"reward_components/{key}/min"] = float(values.min())

    # Track answer length (from <answer>...</answer>) near response_length metrics.
    answer_lengths = batch.non_tensor_batch.get("answer_length", None)
    if answer_lengths is not None:
        values = np.asarray(answer_lengths, dtype=np.float32).reshape(-1)
        values = values[np.isfinite(values)]
        if values.size > 0:
            result["response_length/answer_mean"] = float(values.mean())
            result["response_length/answer_max"] = float(values.max())
            result["response_length/answer_min"] = float(values.min())
    return result
