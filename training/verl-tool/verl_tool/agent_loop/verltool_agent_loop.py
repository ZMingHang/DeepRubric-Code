...
class AgentActorConfig:
    ...
    max_concurrent_trajectories: int=16 # Maximum number of concurrent trajectories globally for async rollout for all agent workers. If None, no limit is applied.
...
class VerlToolAgentLoop(AgentLoopBase):
    ...
    async def run(self, sampling_params: dict[str, Any], **kwargs) -> AgentLoopOutput:
        ...
        if image_data or audio_data:
            ...
            initial_prompt_text = raw_prompt_text
        else:
            initial_prompt_text = self.tokenizer.decode(prompt_ids, skip_special_tokens=False)
        ...
        for step in range(max_turns + 1):
            ...
            with simple_timer("generate_sequences", metrics):
                ...
                if output.text.strip() == "":
                    prompt_preview = initial_prompt_text.replace("\n", " ")[-200:]

                    logger.warning(
                        "Turn %s: Generated empty response for traj_id=%s. prompt_ids length: %s. prompt_preview: %s",
                        step,
                        request_id,
                        len(running_prompt_ids),
                        prompt_preview,
                    )
            ...
            if finish_reason == "stop" and stop_reason:
                for action_stop_token in self.action_stop_tokens:
                    if action_stop_token in stop_reason:
                        # do action – but extract a clean tool block only
                        candidate = (gen_text.split(action_stop_token)[0] + action_stop_token)
                        
                        # grab the first well-formed tool tag
                        # m = re.search(r"<(?:search|browse|scholar)>(.*?)</(?:search|browse|scholar)>", candidate, re.DOTALL)
                        m = re.search(r"<tool_call>(.*?)</tool_call>", candidate, re.DOTALL)
                        if m:
                            action_text = m.group(0).strip()
                            do_action = True
                        break
            # if action_text=="" and do_action == True:
            #     do_action = False
            # sanitize action_text to only keep the first tool block, avoiding stray thought text
            
            # if do_action and action_text:
            #     m = re.search(r"<(?:search|browse|scholar)>(.*?)</(?:search|browse|scholar)>", action_text, re.DOTALL)
            #     if m:
