...
def _is_meaningful_cited_text(text: str) -> bool:
    ...
    lexical_units = len(re.findall(r"[A-Za-z0-9]+(?:[-'][A-Za-z0-9]+)?|[\u4e00-\u9fff]", cleaned))
    ...
    if symbol_ratio > 0.7 and alpha_num_or_cjk < 4:
        return False
    # Short phrase-like spans should not count as meaningful cited claims.
    if lexical_units < 4:
        return False
...
def _extract_valid_citation_positions(response: str, citations: Dict[str, str]) -> List[float]:
    ...
    for match in re.finditer(cite_tag_pattern, response):
        ...
        citation_ids = [citation_id.strip() for citation_id in citation_ids_raw.split(",") if citation_id.strip()]
        if not citation_ids:
            continue
        valid_citation_ids = sorted({citation_id for citation_id in citation_ids if citation_id in citations})
        ...
        for citation_id in valid_citation_ids:
            positions_by_citation_id.setdefault(citation_id, []).append(normalized_position)
...
def _score_citation_distribution(positions: List[float]) -> float:
    if len(positions) == 0:
        return 0.0
    if len(positions) == 1:
        # A single citation cannot be "uniformly distributed".
        return 0.3
...
def _score_citation_count(valid_unique_citation_count: int, max_count_for_full_score: int = 6) -> float:
...
def score_citation_format(claims: Dict[str, List[str]], citations: Dict[str, str], response: str = "") -> float:
    """
    ...
    count_reward = _score_citation_count(len(valid_citations), max_count_for_full_score=6)

    # Keep backward compatibility if caller does not provide response text.
    if response:
    ...
    # Keep base quality as the main component, then modulate by count/distribution.
    format_multiplier = 0.6 + 0.2 * distribution_reward + 0.2 * count_reward
