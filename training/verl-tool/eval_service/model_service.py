...
class ModelService:
    ...
    def __init__(self, model_config: ModelConfig, tool_config: ToolConfig):
        ...
        self.model = None
        self.session = None
        # allow closed-source/remote models without a local tokenizer
        tokenizer_name = getattr(self.model_config, "tokenizer", self.model_config.model)
        ...
        if tokenizer_name:
            try:
                ...
                # fallback: run without tokenizer; caller must accept no token-based truncation
                print(f"[WARN] tokenizer '{tokenizer_name}' unavailable, running tokenizer-free: {e}")
        ...
        if self.tool_config.mtrl_sep is None:
            if self.tokenizer:
                ...
                # simple separator for tokenizer-free mode
                self.tool_config.mtrl_sep = "\n[OBS]: {obs}\n"
    ...
    async def post_process_observations(self, next_obs: List[str], dones: List[bool], valid_action: List[bool], finishs: List[bool]):
        ...
        if self.tokenizer:
            ...
            # tokenizer-free path: truncate by characters and format directly
            processed = []
            for i, obs in enumerate(next_obs):
                if finishs[i] or dones[i]:
                    processed.append("")
                    continue
                if len(obs) > self.tool_config.max_obs_length:
                    obs = obs[-self.tool_config.max_obs_length:] if self.tool_config.truncate_obs_side == 'left' else obs[:self.tool_config.max_obs_length]
                    print(f"[WARNING] OBSERVATION TOO LONG (chars), truncated to {self.tool_config.max_obs_length}")
                if self.tool_config.enable_mtrl:
                    obs = mtrl_sep.format(obs=obs if valid_action[i] else "Your action is not valid, please check the format and try again." + obs)
    ...
    async def _postprocess_responses(self, outputs: torch.Tensor, action_step: int) -> torch.Tensor:
        ...
        if self.tokenizer:
            ...
            for i in range(len(active_responses)):
                finish = True
                for act_end in self.tool_config.action_stop_tokens:
                    act_start = act_end.replace('/','')
                    if act_start in active_responses[i]:
                        active_responses[i] = active_responses[i] + act_end
                        finish = False
                        break
                finishes.append(finish)
            return active_responses, finishes, active_finish_reasons
    ...
    def load_model(self):
        ...
        if self.model_config.model.startswith("openai/") or self.model_config.model.startswith("remote/"):
            num_models = self.model_config.num_models
            base_url = os.getenv("OPENAI_BASE_URL", "http://localhost:8000/v1")
            api_key = os.getenv("OPENAI_API_KEY", "EMPTY")
            remote_model = self.model_config.model.split("/", 1)[1]
            self.model_config.model = remote_model
            self.clients = [
                openai.Client(api_key=api_key, base_url=base_url)
    ...
    async def send_request(self, client, prompts: List[str], model:str, sampling_params: dict) -> str:
        ...
        # print(f"Sending request to {client.base_url} with prompt: {prompts}")
    ...
    async def generate_with_tools_gpt(self, prompts: List[str], sampling_params: dict) -> Tuple[List[str], List[str]]:
        """
        Generate text with tool calls in a multi-turn loop.
    ...
    async def chat_completions_async(self, body: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.tokenizer:
            ...
            # tokenizer-free prompt rendering
        ...
        if self.tokenizer:
            ...
            all_responses, finish_reasons = await self.generate_with_tools_gpt(prompts, sampling_params)
        ...
        if self.tokenizer:
            ...
            # cannot count tokens without tokenizer
            prompt_tokens = None
