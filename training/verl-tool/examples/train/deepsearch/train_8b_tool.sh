...
dataset_name="${DATASET_NAME:-deeprubric}"
...
lr=1e-6
reward_manager="${REWARD_MANAGER:-longform_rubric}"
...
model_pretty_name="${MODEL_PRETTY_NAME:-$(basename "$model_name" | tr '/' '_' | tr '[:upper:]' '[:lower:]')}"
...
export SEARCH_URL="${SEARCH_URL:-http://localhost:8888/retrieve}"
export BROWSE_URL="${BROWSE_URL:-http://localhost:8888/access}"
export SCHOLAR_URL="${SCHOLAR_URL:-http://localhost:8000/search}"
rollout_mode='async'
...
PYTHONUNBUFFERED=1 "$PYTHON_BIN" -m verl_tool.trainer.main_ppo \
    ...
    actor_rollout_ref.actor.ppo_max_token_len_per_gpu=30768 \
    actor_rollout_ref.rollout.max_model_len=28768 \
    actor_rollout_ref.rollout.max_num_batched_tokens=16768 \
    ...
    trainer.project_name="${PROJECT_NAME:-deepsearch}" \
    trainer.experiment_name=$run_name \
    trainer.val_before_train=False \
    trainer.default_hdfs_dir=null \
    trainer.n_gpus_per_node=$n_gpus_per_node \
    trainer.default_local_dir="${CHECKPOINT_DIR:-${ROOT_DIR}/checkpoints/deepsearch/${run_name}}" \
    trainer.nnodes=$n_nodes \
    +trainer.remove_previous_ckpt_in_save=True \
    trainer.save_freq=5 \
    trainer.test_freq=5 \
    trainer.total_epochs=10 \
    > "${LOG_DIR:-${ROOT_DIR}/logs}/${run_name}.log" 2>&1
